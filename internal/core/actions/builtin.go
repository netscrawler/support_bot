...
func (EndAction) Execute(
	...
	if len(cfg.TerminalNodeIDs) == 1 {
		...
		if !ok {
			return registry.ActionOutput{}, fmt.Errorf(
				"pseudo end action: missing output for terminal node %q",
				cfg.TerminalNodeIDs[0],
			)
		}
...
func (a *QueryAction) Execute(
	...
	for _, q := range cfg.Queries {
		...
		if err != nil {
			return registry.ActionOutput{}, fmt.Errorf(
				"query action: execute query %q: %w",
				q.Name,
				err,
			)
		}
